...
@section('content')
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col">
                                        <input type="text" class="form-control" aria-describedby="emailHelp"
                                            placeholder="Slider Name" name="title" value="{{ $slider->title }}">
                                    ...
                                    <div class="col">
                                        <select class="form-control" name="type">
                                            @foreach (sliderTypes() as $type)
                                                <option value="{{ $type }}" @selected($slider->type == $type)>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-md">
                                        ...
                                        <tbody id="slider-container">
                                            @php $sliderInfo = json_decode($slider->sliderInfo->value) @endphp
                                            @foreach ($sliderInfo ?? [] as $index => $item)
                                                <tr class="slider-item">
                                                    <td>
                                                        ...
                                                        <input type="hidden"
                                                            name="slider_info[{{ $index }}][position]"
                                                            class="position" value="{{ $item->position ?? $index }}">
                                                    </td>
                                                    <td>
                                                        <div id="slider-modal-{{ $index }}-wrapper">
                                                            <div class="image-wrapper">
                                                                @if (!empty($item->image))
                                                                <input type="hidden"
                                                                name="slider_info[{{ $index }}][image]"  value="{{ $item->image }}">
                                                                <img src="{{ asset($item->image) }}" class="mr-3 mb-3">
                                                            @endif
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <input type="text" class="form-control"
                                                            name="slider_info[{{ $index }}][text]"
                                                            value="{{ $item->text ?? '' }}" placeholder="Enter title">
                                                    </td>
                                                     <td>
                                                        <input class="form-control"
                                                            name="slider_info[{{ $index }}][description]"
                                                            value="{{ $item->description ?? '' }}">
                                                    </td>
                                                    <td>
                                                        <input class="form-control"
                                                            name="slider_info[{{ $index }}][link]"
                                                            value="{{ $item->text ?? '' }}">
                                                    </td>
                                                   
                                                    <td>
                                                        <button type="button" class="btn btn-primary"
                                                            id="slider-modal-{{ $index }}-btn"
                                                            data-bs-toggle="offcanvas"
                                                            data-bs-target="#slider-modal-{{ $index }}"
                                                            aria-controls="slider-modal-{{ $index }}"
                                                            aria-expanded="false">
                                                            Upload File
                                                        </button>
                                                    </td>
                                                    <td>
                                                        <button class="btn btn-danger w-100 remove_slider"
    ...
    <div id="media-container">
        @foreach($sliderInfo ?? [] as $index => $item)
            @include('admin.components.media.popup', [
                'modalId' => 'slider-modal-' . $index,
                'inputType' => 'single',
                'imageInputName' => 'slider_info[' . $index . '][image]',
            ])
        @endforeach
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        // start index from last loaded row
        let i = document.querySelectorAll("#slider-container .slider-item").length || 0;
        ...
        // Ensure positions are set on load (important for edit)
        setSlidePosition();
});
    </script>
@endpush
